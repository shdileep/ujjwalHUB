...
# The FIXED content we want to inject
...
try:
    ...
    # Identify the block start/end roughly by fixed markers
    start_index = -1
    end_index = -1

    for i, line in enumerate(lines):
        if 'const getDumpYardIconSvg = (size: number = 48) => {' in line:
            start_index = i - 1 # Include the comment above it if possible
            if '// Custom Unique Recycle Icon' not in lines[start_index]:
                 start_index = i # Or just start at the function
            break
    
    # scan for the end of the dumpYardIcon definition
    if start_index != -1:
        for i in range(start_index, len(lines)):
            if 'iconAnchor: [32, 64],' in line and 'popupAnchor: [0, -64]' in lines[i+1] and '});' in lines[i+2]:
                 end_index = i + 3
                 break
            # Fallback search for end of struct
            if 'const dumpYardIcon' in lines[i]:
                 # search forward
                 pass
            if i > start_index + 60: # Limit search to avoid overshooting
                 break
    
    # HARDCODED LINE REPLACEMENT based on visual confirmation from agent (lines 90-131 approx)
    # The user viewed file confirms lines 90 to 130 are the block.
    # 90: // Custom Unique ...
    # 130: popupAnchor: [0, -64]
    # 131: }); (presumed)
    
    # Let's just use slicing based on the known bad strings to be safer than line numbers?
    # No, line numbers from view_file are reliable enough if we verified them recently.
    # Lines 90 to 131 (inclusive) should be replaced.
    
    # We will verify the content before replacing
    print(f"Replacing lines 90-131...")
    
    # Construct new file content
    # Note: lines list is 0-indexed. Line 90 in editor is index 89.
    
    # Let's double check content at index 89
    print(f"Line 90 content: {lines[89]}")
    if "Custom Unique" not in lines[89] and "getDumpYard" not in lines[90]:
         print("WARNING: Line mismatch! Aborting unsafe replace.")
         # Fallback to finding the string indices
    else:
        new_lines = lines[:89] + [fixed_block + '\n'] + lines[131:]
        
        with open(target_file, 'w', encoding='utf-8') as f:
            f.writelines(new_lines)
        print("Successfully replaced content using Python.")
