...
async function diagnose() {
    ...
    const driverUsers = Object.entries(users).filter(([id, u]: [string, any]) => u.role === 'driver');

    console.log(`Found ${driverUsers.length} users with role 'driver'`);

    driverUsers.forEach(([id, u]: [string, any]) => {
    ...
    Object.entries(live).forEach(([id, s]: [string, any]) => {
